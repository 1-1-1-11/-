...
编辑 `config/coffee-price.config.json`，把 `addresses` 改成你的常用地址。若要接入瑞幸官方 CLI 价格源，地址还需要填写 `longitude` 和 `latitude`，因为官方门店查询按经纬度查附近门店。
...
注意：`orderwise:cli-source` 也从 stdin 读取 `{ query, address }`；Windows PowerShell 5.1 下手动管道测试并传参时同样使用双 `--`。如果 `adb.exe` 不在当前 shell 的 PATH 中，可以显式传 `--adb "C:\...\platform-tools\adb.exe"`；默认也会尝试 winget 的 `Google.PlatformTools` 安装目录，并把 ADB 目录注入 Python 子进程 PATH。
...
npm run orderwise:doctor -- --source-kind cli
```

默认 MCP 检查会确认 endpoint 是否能列出 `compare_prices` 工具、`app_device_mapping.json` 是否仍是占位值，以及 `PHONE_AGENT_BASE_URL` / `PHONE_AGENT_MODEL` 是否已配置。CLI 直连检查会额外确认 Python 能加载 `orderwise_agent`、ADB 是否可执行、是否有已授权 Android/云手机设备。
...
如果拿到的是云手机 ADB 地址，可以用一条命令连接设备、写入映射、写入模型配置并启用 CLI 直连源：

```powershell
$env:PHONE_AGENT_API_KEY = "你的 Phone Agent 或模型服务 API key"
npm run orderwise:configure -- --source-kind cli --connect-adb --meituan "10.0.0.10:5555" --source-apps "美团" --orderwise-model-url "http://localhost:4244/v1" --orderwise-model-name "autoglm-phone-9b" --phone-agent-api-key-env PHONE_AGENT_API_KEY --enable-source
```

如果设备已经通过 USB 或 `adb connect` 授权成功，也可以让配置命令自动读取 `adb devices -l` 并启用 CLI 直连源，把 `orderwiseCli` 限定为“美团”：
...
若 `adb` 不在 PATH，先查出实际路径后再加 `--adb`：

```powershell
Get-ChildItem -Path "$env:LOCALAPPDATA\Microsoft\WinGet\Packages" -Recurse -Filter adb.exe
npm run orderwise:configure -- --source-kind cli --connect-adb --adb "C:\Users\你\AppData\Local\Microsoft\WinGet\Packages\Google.PlatformTools_Microsoft.Winget.Source_8wekyb3d8bbwe\platform-tools\adb.exe" --meituan "10.0.0.10:5555" --source-apps "美团" --orderwise-model-url "http://localhost:4244/v1" --orderwise-model-name "autoglm-phone-9b" --phone-agent-api-key-env PHONE_AGENT_API_KEY --enable-source
```
...
$env:ORDERWISE_ADB_PATH = "C:\path\to\platform-tools\adb.exe"
...
瑞幸咖啡官方 CLI 适合作为第一条真实授权价格源：不抓美团/饿了么 H5，不处理验证码，只走用户 token 授权的官方工具。当前桥接只调用 `store`、`product`、`menu` 和 `order preview`，用于拿自取预估到手价；不会调用 `order create`，因此不会自动下单。

推荐使用官方 CLI bridge：它会下载并校验瑞幸官方 CLI，运行官方 `luckin login`，登录完成后启用 `luckinMcp` 并运行专项检查。
...
这条命令会报告 `DEGRADED` 而不是硬失败：微信查价仍可使用本地价格库和城市参考价，但瑞幸官方 CLI 实时自取价会保持未启用。若你要把实时源当作强制验收条件，加 `--require-live`，检查不通过时命令会返回非零退出码。
...
也兼容公开瑞幸 MCP 工具常用的环境变量名：

```powershell
$env:LUCKIN_MCP_ORDER_TOKEN = "你的瑞幸 MCP token"
```

token 读取顺序是：`LUCKIN_MCP_TOKEN` / `LUCKIN_MCP_ORDER_TOKEN` 环境变量、`%USERPROFILE%\.my-coffee\LUCKIN_MCP_TOKEN`、瑞幸官方 CLI 的 `%USERPROFILE%\.luckin\.env`。

不要把 token 写进 Git 仓库。

已实测官方 CLI 的 `store`、`product`、`menu`、`order preview` 都属于授权工具链；即使只查附近门店也需要 `LUCKIN_MCP_ORDER_TOKEN`。这条路线解决的是“不抓 H5、不做人机验证”的问题，不是匿名查价接口。
...
它会检查配置文件、token、地址经纬度、`externalSources.luckinMcp` 是否存在/启用。`FAIL` 代表还不能走瑞幸官方实时价；`WARN` 通常表示 token 和坐标已具备，但 `luckinMcp.enabled` 仍是 `false`。
...
这条命令会把本地配置里的 `luckinMcp.enabled` 改为 `true`，并把旧的 `src/luckin-mcp-source-cli.ts` 配置迁移到官方 CLI 源；不会保存 token。等价的手工配置如下：
...
```powershell
npm run luckin:official-source
```
...
工具只会打开价格榜单中最低的安全 `http/https` 实时可下单链接；`priceBook` 本地价格库和 `cityBenchmark` 城市参考价即使带有品牌官网或参考页 URL，也只会在榜单里显示为“参考页”，不会被当作购买页自动打开。如果最低价候选没有购买链接，或链接是 `javascript:`、`file:` 等非网页 scheme，会跳过并尝试下一个可打开候选。打开页面只负责跳转到购买页，不会自动提交订单。
